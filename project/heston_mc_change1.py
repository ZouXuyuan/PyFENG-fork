...
class HestonMcABC(heston.HestonABC, sv.CondMcBsmABC, abc.ABC):
    ...
    def LocVolCalibrator(self, t, s, p, TT, S0, r):
        lT = len(TT)
        if abs(t - 1) < 10**(-6):
            t = 1 - 10**(-5)
        t_real = 250 * t
        ...
        sign = np.sqrt(max(p[i, 0] + p[i, 1] * (p[i, 2] * (x - p[i, 3]) + np.sqrt((x - p[i, 3])**2 + p[i, 4])), 0)) / np.sqrt(TT[i] / 250)
        sign_1 = np.sqrt(max(p[i + 1, 0] + p[i + 1, 1] * (p[i + 1, 2] * (x - p[i + 1, 3]) + np.sqrt((x - p[i + 1, 3])**2 + p[i + 1, 4])), 0)) / np.sqrt(TT[i + 1] / 250)
        ...
        d1 = ((r + (sig**2) / 2) * t_real / 250 - x) / (sig * np.sqrt(t_real / 250))
        SigmaLV = np.sqrt(max((sig**2 + 2 * t_real / 250 * sig * (sig_T + r * sig_x)) / ((1 + d1 * np.sqrt(t_real / 250) * sig_x)**2 + sig * t_real / 250 * (sig_xx - sig_x - d1 * np.sqrt(t_real / 250) * (sig_x)**2)), 10**(-5)))
    ...
    def cond_spot_sigma(self, texp, var_0):
        ...
        n_dt = len(dt)
        p = np.load(r"C:\Users\Ellen\Desktop\课内\研二下\stochasitc\project\波动率数据\para_svi.npy")
        spot = 3475.8389
        ...
        for i in tqdm(range(n_dt)):
            var_t, avgvar_inc, extra = self.cond_states_step(dt[i], var_t) #????这个函数是在干嘛？？？？都没有看到函数内容
            ...
            X = X  - 0.5 * var_t*dt[i]* L + np.sqrt(var_t*dt[i]*L) * W_1  ##这个L 是啥意思？？
            X1 = np.copy(X)
            ...
            index = np.argsort(X1)
            for k in range(bin):
                ee = np.sum(var_t1[index[(k - 1) * n : k * n]])
    ...
    #         X = X  - 0.5 * var_t*dt[i] + np.sqrt(var_t*dt[i]) * W_1 
