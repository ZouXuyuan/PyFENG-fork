...
class HestonMcABC(heston.HestonABC, sv.CondMcBsmABC, abc.ABC):
    ...
    def cond_spot_sigma(self, texp, var_0):
        ...
        for i in range(n_dt):
            ...
            Z_1 = self.rv_normal(spawn=0)

            # Euler (or Milstein) scheme
            var_t = var_t + self.mr * (self.theta - var_t_) * dt[i] + np.sqrt(var_t_*dt[i]) * self.vov * Z_1
            var_t_ = np.copy(var_t)
            var_t_[var_t_ < 0] = 0  # variance should be larger than zero

            Z_2 = np.random.normal(size=(self.n_path,))          
            W_1 = self.rho * Z_1 + np.sqrt(1 - self.rho**2)*Z_2

            X = X  - 0.5 * var_t_*dt[i] + np.sqrt(var_t_*dt[i]) * W_1 
