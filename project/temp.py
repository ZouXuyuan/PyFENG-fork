...
class HestonMcABC(heston.HestonABC, sv.CondMcBsmABC, abc.ABC):
    ...
    def cond_spot_sigma(self, texp, var_0):
        ...
        spot_cond = ((var_t - var_0) + self.mr * texp * (
                    avgvar - self.theta)) / self.vov - 0.5 * self.rho * texp * avgvar
        spot_cond *= self.rho
        spot_cond += m_corr
...
class HestonMcGlassermanKim2011(HestonMcABC):
    ...
    def x2_cdf_points_aw(self, dt, shape):
        ...
        cdf_i = np.sum(np.sin(hh * x_i * k_grid) / k_grid * self.x2_avgvar_mgf(1j * hh * k_grid, dt, shape).real,
                       axis=0)
